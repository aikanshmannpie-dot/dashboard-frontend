...
@Component({
  selector: "kt-clickout-report",
  templateUrl: "./clickout-report.component.html",
  styleUrls: ["./clickout-report.component.scss"],
})
export class ClickoutReportComponent implements OnInit {
  ...
  searchRange() {
    ...
    this.startDate = this.model1.start
      ...
      : moment(new Date()).format("YYYY-MM-DD");
    if (this.startDate) {
      this.getClickOutReport(this.startDate, this.endDate);
    } else {
      this.getClickOutReport(this.startDate, this.endDate);
    }
  }
  ...
  getClickOutReport(start, end) {
    ...
    // this.auth.getClickOutReport(start, end).pipe(
    //   tap(user => {
    //     if (user) {
    //       this.avaible = true;
