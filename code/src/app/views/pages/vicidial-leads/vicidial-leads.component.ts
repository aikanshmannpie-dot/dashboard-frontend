  ...
  searchRange() {
    ...
    if (isValid) {
      ...
      if (this.startDate && this.endDate && this.isSelected) {
        this.postListOfVicidial(this.startDate, this.endDate, this.isSelected);
      } else {
        ...
        if (data) {
          if (data.status == 200) {
            data.apiData.map((item) => {
              ...
              item.to_date = moment(item.to_date).format("YYYY-MM-DD");
              item.receive_date = moment(item.receive_date).format(
                "YYYY-MM-DD"
              );
            });
  ...
  customFilterPredicate() {
    const myFilterPredicate = (data: any, filter: any): boolean => {
      ...
      return (
        ...
        data.from_date.toString().trim().indexOf(searchString.from_date) !==
          -1 &&
        data.to_date.toString().trim().indexOf(searchString.to_date) !== -1
      );
    };
