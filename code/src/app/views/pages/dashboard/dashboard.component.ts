	...
	customFilterPredicate() {
		const myFilterPredicate = (data: any, filter: any): boolean => {
			...
			return (
				data.Description.toString()
					.trim()
					.indexOf(searchString.Description) !== -1 &&
				data.Total.toString().trim().indexOf(searchString.Total) !== -1
			);
		};
			...
			.pipe(
				tap((data) => {
					if (data) {
						...
						let sourceByReportGraph =
							data.apiData[1].body.graphData;
						let gpReportGraph = data.apiData[2].body.graphData;
						let affiliateByReportGraph =
							data.apiData[3].body.graphData;
						let overAllReportGraph = data.apiData[4].body.graphData;
						let lineChartDataGraph = data.apiData[5].body.graphData;
						let campaignsReportGraph =
							data.apiData[6].body.graphData;
				...
				tap((data) => {
					if (data) {
						...
						Highcharts.chart("container7", {
							...
							title: {
								text:
									"Unique Email Lead Volumes (Coreg/Clickout)",
							},
							xAxis: {
								categories: this.lineChartagraphData.label,
							},
							...
							series: [
								{
									...
									data: this.lineChartagraphData
										.dataychartALLUnicVryfied,
									type: undefined,
								},
								{
									name:
										"Unique Non Verified Email Lead Volumes",
									data: this.lineChartagraphData
										.dataychartALLUnicNonVryfied,
									type: undefined,
								},
								{
