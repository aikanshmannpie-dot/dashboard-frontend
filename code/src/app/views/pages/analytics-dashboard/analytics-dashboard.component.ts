...
export class AnalyticsDashboardComponent implements OnInit {
  ...
  getGenderData() {
    ...
    this.auth.getGenderCount().pipe(tap((res) => {
      if (res && res.status === 200) {
        let total = 0;
        let seriesData = res.apiData.map((x: { sum_male: any, sum_female: any }) => {
  ...
  getAllLeads() {
    ...
    this.auth.getTotalUniqueLeadsAcquirelyAll().pipe(tap((res) => {
      if (res && res.status === 200) {
        let funnelData: [[string, number]] = [["Unique leads", res.apiData.map((x: { sum: any; }) => { return Number(x.sum) })[0]]];
        ...
        this.auth.getTotalUniqueLeadsAcquirelyPartners().pipe(tap((res) => {
          if (res && res.status === 200) {
            funnelData.push(["Unique leads Partners", res.apiData.map((x: { sum: any; }) => { return Number(x.sum) })[0]]);
            chart.series[0].setData(funnelData, true);

            this.auth.getTotalUniqueLeadsAcquirely().pipe(tap((res) => {
              if (res && res.status === 200) {
                funnelData.push(["Unique leads Acquirely", res.apiData.map((x: { sum: any; }) => { return Number(x.sum) })[0]]);
                chart.series[0].setData(funnelData, true);
                this.auth.getTotalUniqueLeadsTeleconnex().pipe(tap((res) => {
                  if (res && res.status === 200) {
                    funnelData.push(["Unique leads Teleconnex", res.apiData.map((x: { sum: any; }) => { return Number(x.sum) })[0]]);
                    chart.series[0].setData(funnelData, true);
