...
export class AuthService {
    ...
    login(email: string, password: string): Observable<any> {
        ...
        return this.http.post<User>(`${API_USERS_URL}/login`, data, { headers: httpHeaders })
    }

    getAffilate(startDate, endDate): Observable<any>  {
        const httpHeaders = new HttpHeaders();
        httpHeaders.set('Content-Type', 'application/json');

        return this.http.get<User>(`${API_USERS_URL}/affiliate-by-report/${startDate}/${endDate}`, { headers: httpHeaders })
            .pipe(
                map((res: User) => {
    ...
    getOverAllReport(startDate, endDate): Observable<any>  {
        const httpHeaders = new HttpHeaders();
        httpHeaders.set('Content-Type', 'application/json');

        return this.http.get<User>(`${API_USERS_URL}/over-all-report/${startDate}/${endDate}`, { headers: httpHeaders })
            .pipe(
                map((res: User) => {
                    console.log("data",res)
    ...
    register(user: User): Observable<any> {
        ...
        return this.http.post<User>(`http://localhost:8034/api/register`, user, { headers: httpHeaders })
            .pipe(
                map((res: User) => {
                    return res;
                }),
